...
class Allocator:
        ...
        def edge_string(self, item1_points_to, item2):
                """Creates graph string edge

                Args:
                    item1_points_to (int/str): from edge
                    item2 (int/str): to edge
        ...
        def detect_basic_blocks(self):
                ...
                last_line = ""
                for line in self.ir:
                        # ":" is used for a label to ignore
                        if ":" in line:
                                solution.append([line, "None"])
                                # if line is a label, it's the start of new block
                                if "label" == line[0:5] and not ("br" in last_line or "goto" in last_line):
                                        # not statement above prevents double increment
                                        current_block += 1
                        # branches or gotos are ends of a block
                        elif "br" in line or "goto" in line:
                                solution.append([line, str(current_block)])
                                current_block += 1
                        # if no end of block, add this line to the current block numb
                        else:
                                solution.append([line, str(current_block)])
        ...
        def control_flow_graph(self):
                ...
                for i, line_and_block in enumerate(self.bb):
                        ...
                        if current_block == None:  # ignore the labels in cfg
                                continue
                        
                        # branches have fall through. Fallthrough edge is handled here
                        elif "br" in current_inst:
                                next_inst = self.bb[i+1][0]
                                next_block = self.bb[i+1][1]
                                # first handle edge case that last instruction in ir is a branch
                                if len(self.bb) == i + 1:  # technically this won't happen, but just in case
                                        label = split_curr_inst[-1]  # goto label is last of instruction
...
# Fill out in report
        ...
        def find_next_bb_indexs(self, current_basic_block):
                """Takes the block you're in and returns blocks that it points to
                
                ex. if 1 -> 2, and 1 is the input, function returns block 2 index
        ...
        def find_first_line_of_bb(self, basic_block):
                """Finds the line number in the IR corresponding to the start of a bb

                Args:
                    basic_block (str): string of basic block number

                Returns:
                    int: line index of first line of bb
                """
                last_bb = None
        ...
        def find_definitions(self):
                ...
                for i, line in enumerate(self.ir):
                        # adds and assigns are only defs
                        if "add" in line or "assign" in line:
                                # parse the instruction and add it to solution
        ...
        def find_live_in(self):
                ...
                # format the deliverable
                for i in range(num_instructions):
                        """THIS IS THE ONLY LINE THAT IS CHANGED FROM ABOVE (D3)"""
        ...
        #         """Helper to find instruction points where var is defined or used

        #         Args:
        #             variable (str): variable
        #             defined (bool, optional): looking for defs or uses. Defaults to False.

        #         Returns:
        #             list: a list of points where variable used or defined
        #         """
        #         if defined:
        ...
        def find_continuous_streaks(self, int_str_list):
                """Helps to check if a variable is in isolated web mid block

                Args:
                    int_str_list (list): list of live points

                Returns:
                    list: tuples of starts and end indexes of this streak
                """
                if not int_str_list:
                        return []
        ...
        # Interference Helper
        def get_live_ins_or_outs(self, instruct_index, live_in=True):
                """Helper to get the list of live_ins or live_outs at a certain point

                Args:
                    instruct_index (str): instruction index as a string
                    live_in (bool, optional): looking for live ins or outs. Defaults to True.

                Returns:
                    list: list of vars at that point
                """
                if live_in:
                        check_for = self.live_in
                else:
                        check_for = self.live_out
                for i in check_for:
                        if i[0] == instruct_index:  # index 0 of i in instruct index
                                return i[1]  # index 1 is list of vars
                        ...
                        for value in values:
                                for key2, values2 in self.webs.items():
                                        ...
                                        if value in values2 and key != key2:
                                                # then check for edge case of var live in/not live out, but next var just live out (no interference)
                                                live_ins = self.get_live_ins_or_outs(value, live_in=True)
                                                live_outs = self.get_live_ins_or_outs(value, live_in=False)
                                                # index the keys to remove the web number
                                                web1_ends = (key[0] in live_ins and key[0] not in live_outs)
                                                web2_starts = (key2[0] not in live_ins and key2[0] in live_outs)
                                                web1_starts = (key[0] not in live_ins and key[0] in live_outs)
                                                web2_ends = (key2[0] in live_ins and key2[0] not in live_outs)
                                                if (web1_ends and web2_starts) or (web1_starts and web2_ends):
                                                        continue  # ignore this edge case, no interference
                                                # otherwise there is interference here
                                                else:
        ...
        def remove_edges_between(self, web1, web2, graph):
                """Takes the names of two webs and clears their connections on the graph

                Args:
                    web1 (str): interfering web
                    web2 (str): interfering web
                    graph (list): list of lists representing graph nodes

                Returns:
                    list: graph with removed edges
                """
        ...
        def get_basic_block_instruction_indexes(self):
                """Finds basic block instruction indexes

                Returns:
                    dict: key is block and vals are indexes as a int
                """
        ...
        def get_basic_block_of_label(self, label):
                """Takes the label and returns the corresponding basic block

                Args:
                    label (str): ex is 'label0'

                Returns:
                    str: basic block string
                """
                for i, line in enumerate(self.bb):
                        # print(line[0][0:-1])
        ...
        def chaitins_graph_coloring(self):
                ...
                # first go through and create a dictionary of nodes (key) with values = list as edges
                ...
                bb_instructs_at = self.get_basic_block_instruction_indexes() # creates dict of bb to instruction indexes
                loops_encountered = []
                line_points_that_are_loops = set()
                # iterate over lines to find which basic blocks are looped by finding 'label' refrences
                for i in range(len(self.bb)):
                        ...
                        elif "label" in instruct[-1] and instruct[-1] in loops_encountered:  # -1 index because label refrence at end
                                basic_block_of_label = self.get_basic_block_of_label(instruct[-1])
                                # now we have a set of instruction indexes that are loops
                                line_points_that_are_loops.update(bb_instructs_at[basic_block_of_label])
                                        
                graph = []  # this guy will hold all of the node data such as edges and spill cost
                ...
                while g_not_r_colored:
                        temp_graph = copy.deepcopy(sorted_graph)  # don't want temp_graph refrencing sorted_graph
                        color_stack = []
                        ...
                        while len(temp_graph) != 0:
                                ...
                                if len(web_details[1]) < 3:  # index one is edge list
                                        # web details has edges trimmed, so add unchanged original node to colorgraph
                                        ...
                                        for connection in spill_this_web[1]:
                                                self.remove_edges_between(web1=spill_this_web[0],
                                                                        web2=connection,
                                                                        graph=sorted_graph)
                                        # label the node as a spill and reset all of the tracking variables
                        ...
                        # initialize what will be the solution color dict
                        temp_solution = {}
                        for key, value in self.webs.items():
                                temp_solution[key] = None
                        potential_color_i = 0
                        # work through the color stack and provide colors
                        while len(color_stack) != 0:
                                web_details = color_stack.pop()[0]
                                # check to make sure that we don't assign same color to adjacent nodes
                                for neighbor in web_details[1]:
                                        if temp_solution[neighbor] == colors[potential_color_i]:
                                                potential_color_i += 1
                                                potential_color_i = potential_color_i % 3
                                # finally assign it the color from the color array
                                temp_solution[web_details[0]] = colors[potential_color_i]
                                
                        g_not_r_colored = False  # phew :)
                                
                # move values from the temp solution to the actual solution
...
exec_sequence = "yyxxyzxywzxwuuxvvuu"
unique_char = set(exec_sequence)  # filter down to unique charecters
unique_char = list(unique_char)  
unique_char.sort()  # sort the charecters by alphabetical order
solution = {}
for u_char in unique_char:
        # find the indexes that the unique charecter resides in order
        indexes = [i for i, char in enumerate(exec_sequence) if char == u_char]
        for index in indexes:
                # ignore the char if it is at the end of the char sequence
                if index + 1 == len(exec_sequence):
                        continue
                else:
                        # if the next char isn't the same charecter
                        if exec_sequence[index + 1] != u_char:
                                # increment the amount of times we've seen this adjacency
                                solution_string = "{}->{}".format(u_char, exec_sequence[index + 1])
                                if solution_string not in solution:
                                        solution[solution_string] = 0
                                solution[solution_string] += 1

q4a = []  # reformat the dicitonary value to the solution
for key, val in solution.items():
        q4a.append([key, str(val)])
...
# begin solution
original_reg_assignments = {"u": 6, "v": 4, "w":3, "x":2, "y": 1, "z":5}

def find_best_remap(start_permutation, permutations_to_ignore = []):
        """Implements a single iteration of differential register remapping

        Args:
            original_permutation (dict): start perm of the iteration
            permutations_to_ignore (list, optional): list of dicts to ignore for this next iteration. Defaults to [].

        Returns:
            list/dict: returns a tuple. First element is the list of perms/costs and second is the lowest cost element
        """
        # first clean up input if it includes "cost" key
        original_permutation = {}
        for key, value in start_permutation.items():
                if key != "cost":
                        original_permutation[key] = value
        permutations_list = [original_permutation]

        # Iterate through all pairs of keys
        for i, key1 in enumerate(original_permutation):
                for j, key2 in enumerate(original_permutation):
                        # Ensure the keys are different
                        if i != j:
                                # Create a new dictionary by swapping the values of the two keys
                                permutation = original_permutation.copy()
                                permutation[key1], permutation[key2] = permutation[key2], permutation[key1]
                                # don't repeat any permutations 
                                if permutation not in permutations_to_ignore and permutation not in permutations_list:
                                        permutations_list.append(permutation)
        
        # now calculate the cost of the permutation
        for i in range(len(permutations_list)):
                reg_aollocation = permutations_list[i]
                cost = 0
                for adj in q4a:
                        # unpack each adjacency
                        from_reg = adj[0][0]  # first index is edge string
                        to_reg = adj[0][-1]  # last item in "x->y" str
                        potential_cost = int(adj[1])
                        
                        from_reg_numb = reg_aollocation[from_reg]
                        to_reg_numb = reg_aollocation[to_reg]
                        numb_ticks = (to_reg_numb - from_reg_numb) % 6  
                        # only include the cost if it is greater than diffw = 2
                        if numb_ticks > 3:
                                cost += potential_cost
                
                permutations_list[i]["cost"] = cost
                
        min_cost = min(permutations_list, key=lambda x: x["cost"])
        return permutations_list, min_cost
        
q4b_a1, q4b_b1 = find_best_remap(original_reg_assignments)
...
# first permutation for funct is result of q4b_b1 (min cost). Now ignore the original reg map
q4b_a2, q4b_b2 = find_best_remap(q4b_b1, [original_reg_assignments])
